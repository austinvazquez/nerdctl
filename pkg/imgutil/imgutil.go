...
func GetUnusedImages(ctx context.Context, client *containerd.Client, filters ...FilterFunc) ([]images.Image, error) {
	var (
	...
	unusedImages := make([]images.Image, len(allImages))
...
func GetDanglingImages(ctx context.Context, client *containerd.Client, filters ...FilterFunc) ([]images.Image, error) {
	var (
		imageStore = client.ImageService()
	)
	...
	filters = append([]FilterFunc{FilterByDangling(true)}, filters...)
